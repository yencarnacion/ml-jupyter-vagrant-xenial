...
def pca(data, k=2):
    ...
    """
    dataCovAuto = estimateCovariance(data)
    # Calculate the eigenvalues and eigenvectors from correlatedCovAuto
    eigVals, eigVecs = eigh(dataCovAuto)
    ...
    y=1
    for x in range(len(data.take(1)[0])):
        if(x < k):
            retvec.append(eigVecs[:, inds[-y]])
        retval.append(eigVals[inds[-y]])
        y = y+1

    retvec_a = np.array(retvec).transpose()
    retval_a = np.array(retval).transpose()
    # Use the topComponent and the data from correlatedData to generate PCA scores 
    dataScores=data.map(lambda x: [x.dot(retvec[y]) for y in range(len(retvec))])

    return (retvec_a, dataScores, retval_a )
